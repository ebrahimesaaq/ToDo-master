...
class TabBarDaily extends StatelessWidget {
  const TabBarDaily({super.key});

  @override
  Widget build(BuildContext context) {
    final db = DbBase();
    final DailyTasksController controller = Get.put(
      DailyTasksController(),
    );
    return Obx(
      () {
        final largeTasks = controller.tasks.where((l) {
          if (!l.containsKey(DbBase.largeTaskTitle) ||
              !l.containsKey(DbBase.largeTaskStartDate) ||
              !l.containsKey(DbBase.largeTaskEndDate)) {
            return false;
          }

          DateTime startDate = DateTime.parse(l[DbBase.largeTaskStartDate]);
          DateTime endDate = DateTime.parse(l[DbBase.largeTaskEndDate]);
          DateTime today = DateFormat('yyyy-MM-dd')
              .parse(DateTime.now().toString().split(' ')[0]);

          bool isWithinRange = (today.isAtSameMomentAs(startDate) ||
              today.isAtSameMomentAs(endDate) ||
              (today.isAfter(startDate) && today.isBefore(endDate)));

          return (((l.containsValue('Daily') || l.containsValue('يومية')) &&
                      isWithinRange) ||
                  ((l.containsValue('Weekly') || l.containsValue('اسبوعية')) &&
                      isWithinRange) ||
                  ((l.containsValue('Monthly') || l.containsValue('شهرية')) &&
                      isWithinRange) ||
                  ((l.containsValue('Yearly') || l.containsValue('سنوية')) &&
                      isWithinRange)) &&
        ...
        final miniTasks = controller.tasks
            .where(
              (m) =>
                  ...
                  (m.containsValue('Daily') || m.containsValue('يومية')) &&
                  m.containsValue(
